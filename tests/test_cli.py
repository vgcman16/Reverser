...
def test_cli_catalog_schemas_output_json(capsys):
    ...
    exit_code = main(["schema", "--kind", "pe-constructor-installs"])
    captured = capsys.readouterr()
    payload = json.loads(captured.out)
    assert exit_code == 0
    ...
    exit_code = main(["schema", "--kind", "pe-registration-records"])
    captured = capsys.readouterr()
    payload = json.loads(captured.out)
    assert exit_code == 0
    assert "ranges" in payload["required"]

    exit_code = main(["schema", "--kind", "pe-selector-table-dispatches"])
    captured = capsys.readouterr()
    payload = json.loads(captured.out)
    assert exit_code == 0
    assert "ranges" in payload["required"]
...
def test_cli_schema_list_outputs_registry(capsys):
    ...
    assert any(item["kind"] == "pe-constructor-installs" for item in payload["schemas"])
